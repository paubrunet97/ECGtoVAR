...
from utils import load_segmentations, temporal_fids, similarity_matrix, onoff_pairs

''' Creates a list of all the acceptable cardiac cycle in each cycle and resamples them all to shared segment lengths
for all the signals. Then it computes the dissimilarity function between all the acceptable cardiac of a signal, which
will be fed to DS3 Subset Selection algorithm to determine the most representative beat of a signal'''
...
BEATon = sak.load_data('_Intermediates/BEATon.csv')
...
for signalname in tqdm(list(Pon)):
    ...
    for beaton, beatoff in zip(BEATon[signalname][1:-1], BEATon[signalname][2:]):
        ...
        basedic['fs'] = 500 # Take care to write the signal's frequency here
        basedic['BEATon'] = beaton
        basedic['BEAToff'] = beatoff

        # Takes fiducials in the beat, merge two consevutive Pon, Poff, Pon, Poff.
        basedic['beat_fids'] = [fid for fid in fids if beaton <= fid[0] < beatoff]
        beat_fids_types = [s[1] for s in basedic['beat_fids']]
        for i in range(len(basedic['beat_fids'])):
            if beat_fids_types[i:i + 4] == ['pon', 'poff', 'pon', 'poff']:
                del basedic['beat_fids'][1:3]
                print(basedic['beat_fids'])
            elif beat_fids_types[i:i + 4] == ['pon', 'pon', 'poff', 'poff']:
                del basedic['beat_fids'][1:3]

        # Discard the second fid if it is a toff (beat before that had TP overlap)
        if len(basedic['beat_fids']) >= 2 and (basedic['beat_fids'][0][1], basedic['beat_fids'][1][1]) == ('pon', 'toff'):
            basedic['beat_fids'].pop(1)

        # List fid type of fiducials in the beat
        beat_fids_types = [beat_fid_name[1] for beat_fid_name in basedic['beat_fids']]
        ...
        if len(beat_fids_types) in [5,6]:
            ...
            if beat_fids_types == ['pon', 'poff', 'qrson', 'qrsoff', 'ton', 'toff']:
                ...
                basedic['Pon']    = int(basedic['beat_fids'][0][0])
                basedic['Poff']   = int(basedic['beat_fids'][1][0])
                basedic['QRSon']  = int(basedic['beat_fids'][2][0])
                basedic['QRSoff'] = int(basedic['beat_fids'][3][0])
                basedic['Ton']    = int(basedic['beat_fids'][4][0])
                basedic['Toff']   = int(basedic['beat_fids'][5][0])
                ...
                OverlapDic['NOoverlap'].append((signalname, beaton, basedic['beat_fids']))
            ...
            # 'PQ overlap'
                ...
                basedic['Pon']    = int(basedic['beat_fids'][0][0])
                basedic['Poff']   = int(basedic['beat_fids'][2][0])
                basedic['QRSon']  = int(basedic['beat_fids'][1][0])
                basedic['QRSoff'] = int(basedic['beat_fids'][3][0])
                basedic['Ton']    = int(basedic['beat_fids'][4][0])
                basedic['Toff']   = int(basedic['beat_fids'][5][0])
                ...
                OverlapDic['PQoverlap'].append((signalname, beaton, basedic['beat_fids']))
            ...
            # 'PQ + ST overlap'
                ...
                basedic['Poff']   = int(basedic['beat_fids'][2][0])
                basedic['QRSon']  = int(basedic['beat_fids'][1][0])
                basedic['QRSoff'] = int(basedic['beat_fids'][4][0])
                basedic['Ton']    = int(basedic['beat_fids'][3][0])
                basedic['Toff']   = int(basedic['beat_fids'][5][0])
                ...
                OverlapDic['PQ+SToverlap'].append((signalname, beaton, basedic['beat_fids']))
            ...
            # 'PQ + TP overlap'
                ...
                OverlapDic['PQ+TPoverlap'].append((signalname, beaton, basedic['beat_fids']))
            ...
            # 'ST overlap'
                ...
                OverlapDic['SToverlap'].append((signalname, beaton, basedic['beat_fids']))
            ...
            # 'ST + TP overlap'
                ...
                OverlapDic['ST+TPoverlap'].append((signalname, beaton, basedic['beat_fids']))
            ...
            # 'TP overlaps'
                ...
                OverlapDic['TPoverlap'].append((signalname, beaton, basedic['beat_fids']))
            ...
            # 'PQ + ST + TP overlaps'
                ...
                OverlapDic['PQ+ST+TPoverlap'].append((signalname, beaton, basedic['beat_fids']))

            else:
                basedic['AcceptableBeat'] = False
                OverlapDic['Weirdos'].append((signalname, beaton, basedic['beat_fids']))

        else:
            basedic['AcceptableBeat'] = False
            OverlapDic['Weirdests'].append((signalname, beaton, basedic['beat_fids']))


        # (Criterion 2) Check that baseline does not wonder too much in zones which should remain electrically silent:
        signal = pd.read_csv('ECGs/Adol_denoised/' + signalname + '.csv', sep=',', header=None).to_numpy()

        # TP period does not have voltages > 0.2V
        if basedic['AcceptableBeat'] and abs(signal[basedic['Toff']:basedic['BEAToff'], :]).max() > 0.2:
            basedic['AcceptableBeat'] = False
            OverlapDic['TPwonder'].append((signalname, beaton, basedic['beat_fids']))

        # QRSon, QRSoff does not have voltages > 0.2V
        if basedic['AcceptableBeat'] and (abs(signal[basedic['QRSon']-5 , :]).max() > 0.2 or
                                          abs(signal[basedic['QRSoff']+5, :]).max() > 0.2):
            basedic['AcceptableBeat'] = False
            OverlapDic['QRSwonder'].append((signalname, beaton, basedic['beat_fids']))

        # Pon, Poff does not have voltages > 0.2V
        if basedic['AcceptableBeat'] and (abs(signal[basedic['Pon'] , :]).max() > 0.2 or
                                          abs(signal[basedic['Poff'], :]).max() > 0.2):
            basedic['AcceptableBeat'] = False
            OverlapDic['Pwonder'].append((signalname, beaton, basedic['beat_fids']))

# Some metrics on the Acceptable Beats
lengths = []; acceptable_beats=[]
for signal in BeatsDic:
    beats_signal = 0
    for beat in BeatsDic[signal]:
        if BeatsDic[signal][beat]['AcceptableBeat']:
            lengths.append(BeatsDic[signal][beat]['lengths'].tolist())
            beats_signal += 1
    acceptable_beats.append(beats_signal)

lengths = list(map(list, zip(*lengths)))
print('Number of acceptable beats: ' + str(len(lengths[0])))
print('Acceptable beats per signal: ' + str(round(np.mean(acceptable_beats), 2)) + '±' + str(round(np.std(acceptable_beats), 2)))
...
for signalname in tqdm(list(BeatsDic)):
    ...
    #Load signals and sampling frequency
    signal = pd.read_csv(basedir + signalname + '.csv', sep=',', header=None).to_numpy()
    fs=500 # Take care to write the signal's sampling frequency here

    # Resample each segment to have an equal length
    resampled_beats = []
    for beaton, beatoff in zip(BEATon[signalname][1:-1], BEATon[signalname][2:]):
        ...
        if basedic['AcceptableBeat']:
            ...
            # Concatenate all segments and append it to a nice list
            ...
            # The resmpled_beats is a list of resampled_beat, along its pre-re-sampling lengths.
            basedic['resampled_beat'] = resampled_beat
            basedic['original_beat'] = signal[beaton:beatoff, :]
