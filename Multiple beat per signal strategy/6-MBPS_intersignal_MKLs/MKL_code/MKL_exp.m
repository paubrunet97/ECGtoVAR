function [F_data,betas,A] = MKL_exp(FEATURES, options, resume, resume_iterations,T_kernels)
...
if ~resume
    ...
    parfor c=1:n_features 
        ...
        tmpK = Kernel_Calculus(tmpF,options.Kernel{c}.KernelType, options.Kernel{c}.Parameters,alpha).^T_kernels; %%% a single function only, remove "Kernel_Calculus"
        tmpK2 = Kernel_Calculus(tmpF2,options.Kernel{c}.KernelType, options.Kernel{c}.Parameters,alpha).^T_kernels;
        KERNELS(c,:,:) = tmpK;
        KERNELS2(c,:,:) = tmpK2;
        K_var(c) = var( tmpK(:) );
    end    
    ...
    W = Affinity_matrix(K_sum, options.AffinityNN).^T_kernels; 
    ...
    figure('units','normalized','position',[0 0 1 1],'name','Visualization of Kernels');
    for c=1:n_features
        subplot(1,n_features,c); hold on;
        title(['Kernel ', num2str(c)]);
        imagesc( squeeze(KERNELS2(c,:,:)) );
        axis equal; axis off; axis ij;
    end

    figure('units','normalized','position',[0 0 1 1],'name','Visualization of Global Affinity Matrix');
    ...
    conv_crit = 1e-10;%1e-6                           % Convergence criterion
...
while (t <= max_it) && (abs(gap_old-gap) > conv_crit)
    ...
    %%%-----------Optimization of A (projection matrix)--------------
