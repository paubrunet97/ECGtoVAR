...
def get_variability_descriptors(embedding: np.ndarray, Features: List[Union[np.ndarray, list]],
                                dimensions: Union[None, int, list] = None, direction: np.ndarray = None,
                                gamma: Union[float, np.ndarray] = None, max_iterations: int = 20,
    ...
    * NN_dims: int
    ...
    embedding_distance_density = embedding_distance + np.diag(
        np.full((embedding_reduced.shape[0],), np.inf))  ## put diagonal coefficients to -1
...
def process_table(data: pd.DataFrame, clusters: np.ndarray, y: Union[str,np.ndarray,list] = "outcome", dtypes: dict = {}):
    ...
    for i,k in enumerate(data):
        ...
        # Get non-NaN values
        if var_types[k] == bool:
            non_nan_data = data[k][~np.isnan(data[k])].values
        ...
        for c in np.unique(clusters):
            ...
            if var_types[k] == bool:
                non_nan_cluster_data = cluster_data[~np.isnan(cluster_data)]
